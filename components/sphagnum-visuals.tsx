...
/**
 * Потолок смещения в пикселях ИСХОДНОЙ картинки.
 *
 ...
 * больше единицы.
 *
 * Было 5, и это оказалось слишком тихо, чтобы эффект вообще заметили: кадр
 * ужимается примерно в 0.52 (2752 px исходника на 1440 экрана), так что 5
 * превращались в 2.6 экранных пикселя — на густой листве такой сдвиг глаз не
 * ловит. Пересчёт идёт в ЭКРАННЫЕ пиксели множителем cover, поэтому число тут
 * всегда надо делить на этот масштаб, а не читать как экранное.
 *
 * Верхняя граница остаётся: за ~15 экранных пикселей фотография начинает
 * плыть резиной, и эффект мгновенно читается дешёвым. 24 × 0.52 ≈ 12.5 px —
 * заметно, но ещё в пределах шелеста.
 */
const WALL_AMP_IMAGE_PX = 24;
/**
 * Радиус спада — доля меньшей стороны холста. Возмущение локальное, «под
 * ладонью». Было 0.2: пятно получалось меньше ладони, и, ведя курсор, человек
 * задевал слишком мало листвы, чтобы связать движение с собой.
 */
 ...
 * Было 2600 — это быстрый рывок через весь экран; при спокойном ведении мыши
 * энергии набиралось едва на треть, и стена почти не отзывалась.
 */
