 ...
 */
const HERO_BG_FILTER = "brightness(.80) saturate(1.20) contrast(1.08)";
...
export default function SphagnumLanding() {
  ...
  // Параллакс фона героя считается от прокрутки в rAF-обработчике: сырой
  // scroll-листенер дёргал бы layout на каждом событии, а не раз за кадр.
  useEffect(() => {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      // При отключённом движении параллакс фона не запускаем вовсе.
      return;
    }
    let frame = 0;
    const onScroll = () => {
      ...
      */}
      ...
      <section id="top" ref={heroRef} className="hero-pin relative">
        ...
        <div
          ...
          // Привязка к правому верхнему углу, а не по центру: у кадра
          // соотношение 1.79, у контейнера с запасом — около 1.14, и при cover
          // по бокам срезается заметно. Центрирование съедало правый край
          // вместе с золотым знаком на стене; сверху при этом терялась
          // стеклянная кровля. right top оставляет в кадре и то, и другое.
          //
          // На узком экране привязка ДРУГАЯ. Там cover срезает по ширине почти
          // всё — видно около 18% кадра, и справа это тёмная стена со знаком:
          // вместо стены получался бы размытый логотип во весь экран. Слева
          // сплошная листва, поэтому до lg кадр приколот к левому краю.
          // Тот же порог продублирован в LivingWall (matchMedia 1024px) —
          // меняете здесь, меняйте и там, иначе холст прыгнет относительно
          // подложки в момент появления.
          className="absolute -inset-y-[20%] inset-x-0 bg-cover bg-left-top bg-no-repeat will-change-transform lg:bg-right-top"
          style={{
            backgroundImage: `url("${receptionWall.src}")`,
            // Прежние brightness(.62) saturate(.72) держали фон бледным — человек
            // посмотрел вживую и попросил ярче и сочнее. saturate(.72) срезал
            // насыщенность почти на треть, а не только приглушённая яркость была
            // виновата. contrast(1.08) добавлен, чтобы поднятая насыщенность не
            // выглядела плоской.
        ...
        {/*
          ...
          шевелится под курсором. Классы, фильтр и параллакс ОБЯЗАНЫ совпадать со
        ...
        */}
        ...
        {/*
          ...
          Правее, где текста нет, пелены НЕТ вовсе: .06 к 78% и 0 к 92%. Там
          сцена идёт в полную силу — освещённая стена, стеклянная кровля и
          золотой знак, который перекликается с золотой кнопкой в тексте.
          Ради этого правый край и не затемняется совсем.
        ...
        */}
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(100deg, rgba(9,26,17,.90) 0%, rgba(9,26,17,.84) 46%, rgba(9,26,17,.06) 78%, rgba(9,26,17,0) 92%)",
          }}
        />
