...
/*
  ...
  Преломление держится на геометрии излома, а не на подкраске: расщепление по
  спектру пробовали и убрали — столбы света от него теряли форму.
*/
...
/*
  Заливка ВЕРТИКАЛЬНАЯ и одноцветная — как была изначально: чистый столб света,
  гаснущий книзу. Горизонтальный радужный градиент (тёплая кромка слева,
  холодная справа) физически изображал дисперсию, но лучи от него переставали
  читаться столбами и выглядели плёнкой.

  Затухание РАЗРЕЗАНО между двумя отрезками: .ray-a идёт от .62 до .30,
  .ray-b подхватывает с .30 и уходит в ноль. Иначе каждый отрезок гас бы
  самостоятельно, и на стыке вспыхивала бы яркая перемычка.
*/
.sph .light-rays .ray-a {
  height: 62%;
  opacity: var(--ro);
  background: linear-gradient(180deg, rgba(186,225,75,.62) 0%, rgba(186,225,75,.3) 100%);
}
...
.sph .light-rays .ray-b {
  ...
  transform-origin: 50% 0;
  filter: blur(8px);
  background: linear-gradient(180deg, rgba(186,225,75,.3) 0%, rgba(186,225,75,0) 82%);
