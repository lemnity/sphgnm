...
.sph {
  ...
  */
  ...
  /*
    ЗОЛОТО — осознанное исключение из палитры, по прямой просьбе.
    В гайдбуке его нет, поэтому область применения ЗАКРЫТАЯ, всего три места:
    прорастающий знак в фоне первого экрана, кнопка героя и полоса «53,000 km²».
    Четвёртое место — и это уже не акцент, а пятая краска в четырёхцветной
    палитре.

    ВАЖНО РАЗЛИЧАТЬ ДВЕ РОЛИ, их путали:
    · золото как ТЕКСТ живёт только на тёмном (по Ink 8.4:1, по Cream 1.9:1) —
      на светлых секциях золотых надписей быть не должно;
    · золото как ЗАЛИВКА работает на светлых секциях, потому что текст по нему
      идёт тёмный: Ink по золоту 8.8:1, ink-85 — 6.6:1. Так устроены и .btn-gold,
      и полоса .band-gold.

    Оттенок тёплый и приглушённый (не жёлтый металлик): знак лежит на зелени
    мха и под тёплым белёсым светом лучей, и чистое золото рядом с ними
    выглядит наклейкой. Прозрачность 62% — знак остаётся фоновой графикой и не
    спорит с заголовком.

    gold-light / gold-deep — крайние стопы градиента полосы, НЕ самостоятельные
    цвета: оба выбраны по контрасту (см. .band-gold), а не подобраны на глаз.
    gold-line — волосяная линейка внутри полосы. Она затемнение Ink, а не третий
    оттенок золота: линия из золота по золоту не видна.
  */
  --brand-gold: #D7B15E;
...
/*
  ── ЗОЛОТАЯ ПОЛОСА (акцентный блок «53,000 km²») ──
  Не плоская заливка. Сплошной #D7B15E во всю ширину 1460+ при высоте в треть
  экрана читается пластиковой наклейкой: глаз ждёт от золота хоть какого-то
  перелива. Градиент держится ВНУТРИ золотого тона — светлее в левом верхнем
  углу, глубже к правому нижнему, — поэтому полоса выглядит фольгой, а не
  вторым цветом в палитре.

  Крайние стопы выбраны РАСЧЁТОМ контраста, а не на глаз (замер по всем трём
  стопам, а не по среднему тону):
    · Ink (число, подзаголовок): 10.31 / 8.82 / 7.70 — AAA везде;
    · ink-85 (абзац и факты):     7.50 / 6.63 / 5.94 — AA везде.
  То есть текст проходит порог в ЛЮБОЙ точке полосы, включая тёмный угол
  градиента. Стопы поэтому нельзя «чуть затемнить для красоты»: ink-85 по
  самому глубокому уже 5.94:1, и запас до 4.5:1 меньше, чем кажется.

  Прозрачный стоп записан как rgba того же золота с нулевой альфой, а не
  ключевым словом transparent: transparent — это прозрачный ЧЁРНЫЙ, и часть
  браузеров интерполирует через серый, давая грязную кайму по диагонали.
*/
.sph .band-gold {
  background:
...
/*
  ── СОЛНЕЧНЫЕ ЛУЧИ ──
  Сами лучи рисует фрагментный шейдер на холсте (см. LightRays в
  sphagnum-visuals.tsx). Здесь остались только контейнер и пылинки.

  mix-blend-mode: screen — свет должен ПРИБАВЛЯТЬСЯ к тёмному фону. Холст
  непрозрачно чёрный там, где света нет, а screen с чёрным ничего не меняет,
  поэтому чёрный прямоугольник кадр не портит. Обычной полупрозрачной
  заливкой вместо screen получались грязно-серые полосы вместо свечения.

  Тут была целая механика из шести столбов на CSS: два отрезка в потоке,
  clip-path-клин, маска-колокол, просветы кроны градиентом, нахлёст против
  ступеньки на стыке и размытие на родителе. Её съел шейдер: яркость там
  считается для каждого пикселя, а не собирается из фигур.
*/
/*
  Моховая стена. Холст рисует полупрозрачные штрихи, а screen прибавляет их к
  фотографии под ним: мох прорастает СКВОЗЬ кадр, а не лежит плёнкой поверх.
  При обычном наложении светлые штрихи забеливали фотографию до молока.
*/
...
/*
  ── ПЫЛИНКИ В СТОЛБАХ ──
  Столб света видно только потому, что в нём есть взвесь: сам по себе воздух
  прозрачен. Поэтому пылинки тут не украшение, а то, из-за чего свет читается
  ОБЪЁМНЫМ, а не наложенным поверх кадра градиентом.

  Ореола НЕТ намеренно. Как раз ореол делает точку светлячком (см. .firefly
  ниже — там он прописан прямо), а нужна пылинка на грани видимости.

  Движение медленное и почти без падения: пыль в воздухе плавает, а не сыплется.
  Кадры 0% и 100% гасят её в ноль, поэтому скачок сноса на стыке петли не виден.
*/
.sph .mote {
  ...
  /*
    will-change ЗДЕСЬ НЕ СТАВИТЬ. Кажется, что двенадцати анимированным точкам
    он поможет, а на деле переводит их в двенадцать постоянных слоёв, и все они
    лежат внутри группы mix-blend-mode: screen — группу приходится собирать из
    двенадцати текстур каждый кадр. Замер прокрутки: 98 кадров за 2.2 с с
    will-change против 124 без него, медиана кадра 16.1 против 9.1 мс.
  */
}
@keyframes sphMote {
...
/*
  ── ЛЕСЕНКА ВНУТРИ ОТКРЫВШЕГОСЯ БЛОКА ──
  Дети .stagger выезжают по очереди, но ТОЛЬКО после того, как родительский
  .reveal получил .in. Селектор именно такой (.reveal.in .stagger > *), и это
  принципиально: повесь анимацию прямо на детей — она отыграет при монтировании,
  пока родитель ещё прозрачен, и к моменту появления блока лесенка будет уже
  закончена. Пользователь увидит статичный список.

  Задержки заданы до пяти позиций — больше пяти строк в лесенке не бывает ни в
  одном блоке; шестая и дальше просто выедут вместе с пятой.
*/
...
/*
  ── ОБРАЗЕЦ МХА ПАРИТ ──
  Уступ вырезан по контуру и висит над собственной тенью: неподвижный он читается
  наклейкой, а не предметом. Две петли одного периода, но в ПРОТИВОФАЗЕ — тень
  сжимается и бледнеет, когда объект уходит вверх. Это и есть то, что делает
  парение убедительным; синхронные петли выглядели бы дрожанием картинки.

  Период длинный (7.5с) намеренно: движение должно читаться боковым зрением как
  дыхание, а не привлекать внимание — рядом текст, который надо читать.

  Тень обязана нести translateX(-50%) прямо в keyframes: у неё же есть класс
  Tailwind -translate-x-1/2, и анимация перебивает свойство transform целиком.
  Без сдвига внутри keyframes тень на время анимации уехала бы вправо на пол-ширины.
*/
...
@media (prefers-reduced-motion: reduce) {
  ...
  /* Лесенка гаснет вся: animation:none снимает и заливку both, поэтому строки
     остаются с естественной непрозрачностью 1, а не залипают на нулевом кадре
     keyframes (он стартует с opacity 0). Это как раз тот случай, где «выключить»
     безопасно — в отличие от светлячков ниже. */
  .sph .reveal.in .stagger > * { animation: none; }
  /* Образец мха перестаёт парить и садится на тень. Тень при этом обязана
     сохранить сдвиг: он живёт в классе Tailwind, и без анимации применяется он. */
  ...
  /* Лучей здесь больше нет: их рисует шейдер, и «меньше движения» он
     обрабатывает сам — рисует один кадр и останавливает цикл (см. LightRays).
     Гасить их отсюда нечем и не нужно. */
  .sph .wb-wave, .sph .wb-drop { animation: none; }
  .sph .wb-bubble { animation: none; opacity: 0; }
  /* Пылинки ПРЯЧУТСЯ по той же причине, что светлячки и пузырьки: их keyframes
     стартуют с opacity 0, и без анимации в воздухе повисла бы неподвижная
     россыпь точек — уже не пыль в луче, а мусор на экране. */
